...
type FailureOutcome = 'roomNotFound' | 'connectionFailed' | 'alreadyInProgress' | 'timeout';
...
// link (see Part 3 of BRIEF.md) - same underlying attempt regardless of how
...
export class ConnectingScene extends Phaser.Scene {
  ...
  create(data: ConnectingSceneData): void {
    ...
    const succeed = (toButtonScreen: boolean, hostPeerId: string): void => {
      if (settled) return;
      settled = true;
      if (timer) clearTimeout(timer);
      ...
      this.scene.start(toButtonScreen ? 'PlayerButton' : 'PlayerLobby', sessionData);
    };
    ...
    void data.iceServersPromise.then((iceServers) => {
      ...
      // calls, so PlayerLobby/PlayerButton reassigning `.onMessage` on this
